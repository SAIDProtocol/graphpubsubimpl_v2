...
int GPS_ARPTable::parseArgFile(const String &fileName, ErrorHandler *errh,
                               std::unordered_map<gps_na_t, GPS_NAAddress> &naAddresses) {
    ...
    while (getline(&line, &len, fp) != -1) {
        ...
        naAddresses[na] = GPS_NAAddress(port, ether, ip);
    }
    fclose(fp);
    if (line) free(line);
    return 0;
...
void GPS_ARPTable::handleData(Packet *packet) {
    auto na = GPS_ANNO_NEXT_HOP_NA(packet);
    ...
    if (unlikely(gps_na_is_broadcast(na))) { // broadcast
        ...
        for (auto &it : _naAddresses) {
            ...
            *reinterpret_cast<EtherAddress *> (GPS_ANNO_NEXT_HOP_ETHER(p)) = val.getEther();
            *reinterpret_cast<IPAddress *> (GPS_ANNO_NEXT_HOP_IP(p)) = val.getIp();
            *GPS_ANNO_NEXT_HOP_PORT(p) = static_cast<uint8_t>(val.getPort());
        ...
        if (unlikely(it == _naAddresses.end())) { // no neighbor with NA
            ...
            auto &val = it->second;
            *reinterpret_cast<EtherAddress *> (GPS_ANNO_NEXT_HOP_ETHER(packet)) = val.getEther();
            *reinterpret_cast<IPAddress *> (GPS_ANNO_NEXT_HOP_IP(packet)) = val.getIp();
            *GPS_ANNO_NEXT_HOP_PORT(packet) = static_cast<uint8_t>(val.getPort());
            _lock.release_read();
